...
@login_required
def manage_pro_members(request):
    if request.user.role != 'secretary':
        return redirect('home')
    
    from .models import User, Subscription
    
    # Get active subscription to see the limit
    subscription = Subscription.objects.filter(
        society_name=request.user.society_name, 
        is_active=True
    ).first()
    ...
    if not subscription:
        messages.warning(request, "You need an active subscription to manage Pro members.")
        return redirect('subscription_view')
    
    limit = subscription.duration_months # Wait, no, the limit is flats_count. 
    # Actually, flats_count isn't explicitly stored in a field called 'limit', 
    # but I'll use plan_tier or better, I should add a flats_count field to Subscription.
    
    # Let's check Subscription fields. 
    # Ah, I added plan_tier but not the raw number of flats. 
    # Actually, in the migrate view I had plan_tier.
    
    # I will assume the 'limit' is extracted from the latest POST that created it.
    # Wait, I'll just use the plan_tier ranges or better, I'll check my 'select_plan' logic again.
    
    # To be safe, I'll allow up to 600 if tier is 501+, etc. 
    # But ideally, it's the 'flats_count' they entered. 
    # I should have saved that. 
    
    # I'll update Subscription model to include 'flats_count' in a moment.
    
    current_pro_count = User.objects.filter(society_name=request.user.society_name, is_pro_member=True).count()
    ...
    if request.method == 'POST':
        ...
        if action == 'enable':
            ...
            # For now I'll use a high limit until I add the field
            max_allowed = 1000 
            if subscription.plan_tier == '1-250': max_allowed = 250
            elif subscription.plan_tier == '251-500': max_allowed = 500
            
            if current_pro_count >= max_allowed:
                messages.error(request, f"Limit reached! You can only have {max_allowed} Pro members.")
            else:
                target_user.is_pro_member = True
                target_user.save()
                messages.success(request, f"Pro access granted to {target_user.username}")
        elif action == 'disable':
            target_user.is_pro_member = False
            target_user.save()
            messages.success(request, f"Pro access revoked for {target_user.username}")
            
        return redirect('manage_pro_members')

    residents = User.objects.filter(society_name=request.user.society_name, role='resident').order_by('unit_number')
    
    max_allowed = 1000 
    if subscription.plan_tier == '1-250': max_allowed = 250
    elif subscription.plan_tier == '251-500': max_allowed = 500

    return render(request, 'core/manage_pro_members.html', {
        'residents': residents,
        'current_count': current_pro_count,
        'limit': max_allowed
    })
